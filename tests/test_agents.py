...
def test_arbitrage_agent(monkeypatch):
    agent = ArbitrageAgent(
        threshold=0.1, amount=5, feeds=[fake_feed_low, fake_feed_high]
    )
    actions = asyncio.run(agent.propose_trade("tok", DummyPortfolio()))
...
def test_agent_swarm_weighted(monkeypatch):
    ...
    actions = asyncio.run(
        swarm.propose("tok", DummyPortfolio(), weights={"a": 1.0, "b": 2.0})
    )
    assert actions == [
        {
            "token": "tok",
...
def test_memory_agent(monkeypatch):
    ...
    asyncio.run(
        mem_agent.log({"token": "tok", "side": "buy", "amount": 1.0, "price": 2.0})
    )
