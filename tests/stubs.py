...
def stub_sqlalchemy() -> None:
    ...
    class Query:
        ...
        def filter_by(self, **kwargs):
            self.filters.append(
                lambda obj: all(getattr(obj, k) == v for k, v in kwargs.items())
            )
            return self
    ...
    class AsyncSession:
        ...
        def _execute_text(self, sql: str, params: dict):
            ...
            if low.startswith("insert into trades") and Trade is not None:
                m = re.search(
                    r"insert\s+into\s+trades\s*\(([^)]*)\)\s*values\s*\(([^)]*)\)", low
                )
                if m:
            ...
            if low.startswith("update trades") and Trade is not None:
                m = re.search(
                    r"update\s+trades\s+set\s+(\w+)\s*=\s*:([a-z_]+)\s+where\s+(\w+)\s*=\s*:([a-z_]+)",
                    low,
                )
                rowcount = 0
                if m:
            ...
            if low.startswith("select") and "from trades" in low and Trade is not None:
                m = re.search(
                    r"select\s+([^\s]+(?:\s*,\s*[^\s]+)*)\s+from\s+trades(?:\s+where\s+(\w+)\s*=\s*:([a-z_]+))?",
                    low,
                )
                if m:
                    cols = [c.strip() for c in m.group(1).split(",")]
                    ...
                    rows = []
                    for obj in self.engine.storage.get(Trade, []):
                        if cond_col is not None and getattr(
                            obj, cond_col
                        ) != params.get(cond_key):
                            continue
...
def stub_sklearn() -> None:
    ...
    kernels.__spec__ = importlib.machinery.ModuleSpec(
        "sklearn.gaussian_process.kernels", None
    )
