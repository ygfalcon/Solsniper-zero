...
def test_launcher_writes_ok_markers(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("solhunter_zero.macos_setup.ensure_tools", lambda **k: None)
    monkeypatch.setattr("solhunter_zero.bootstrap_utils.ensure_venv", lambda *a, **k: None)
    ...
    import types
    monkeypatch.setitem(sys.modules, "solhunter_zero.env_config", types.SimpleNamespace(configure_startup_env=lambda root: None))
