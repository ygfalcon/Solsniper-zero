...
if importlib.util.find_spec("sklearn") is None:
    ...
    sys.modules["sklearn.gaussian_process"] = types.SimpleNamespace(
...
async def test_rl_daemon_updates_and_agent_reloads(tmp_path, monkeypatch, caplog, algo):
    ...
    daemon = RLDaemon(
        memory_path=mem_db, data_path=str(data_path), model_path=model_path, algo=algo
    )
...
def test_daemon_receives_risk_update(tmp_path):
    ...
    daemon = RLDaemon(
        memory_path=mem_db, data_path=str(data_path), model_path=tmp_path / "model.pt"
    )
...
async def test_rl_checkpoint_event_emitted(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        RLDaemon, "_fetch_new", lambda self: ([dummy_trade], [dummy_snap])
    )
    daemon.train()
    unsub()
...
def test_predict_action_returns_vector(tmp_path, monkeypatch):
    ...
    daemon = RLDaemon(
        memory_path=mem_db, data_path=str(data_path), model_path=tmp_path / "m.pt"
    )
