"""Bootstrap environment checks for SolHunter Zero."""
...
def log_startup_failure(message: str) -> None:
    """Append ``message`` to ``startup.log`` if possible."""
...
def check_disk_space(min_bytes: int) -> None:
    ...
    try:
        ...
        msg = f"Unable to determine free disk space: {exc}"
        print(msg)
        log_startup_failure(msg)
        raise SystemExit(1)
