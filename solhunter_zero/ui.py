...
def create_app(auto_start: bool = True) -> Flask:
    ...
    ----------
    auto_start:
        When ``True`` (default), automatically invoke :func:`start` after
        initialisation if a keypair and configuration are present.  This can
        be disabled to initialise the UI without launching the trading loop.
    """
    ...
    if active_keypair and active_config:
        ...
        if auto_start:
            startup_message += " Starting trading."
            ...
            startup_message += " Trading start suppressed."
            logger.info(startup_message)
    else:
        missing: list[str] = []
    ...
    if auto_start and os.getenv("AUTO_START") == "1":
