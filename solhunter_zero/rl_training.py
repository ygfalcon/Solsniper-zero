...
class _TradeDataset(Dataset):
    ...
    def __init__(self, trades: Iterable[Any], tokens: Iterable[str], sims_per_token: int = 10) -> None:
        self.samples: List[Tuple[List[float], int, float]] = []
        for t in trades:
            state = [float(t.price), float(getattr(t, "amount", 0.0)), 0.0]
        ...
        for tok in set(tokens):
            try:
                sims = run_simulations(tok, count=sims_per_token, days=1)
            except Exception:
                sims = []
            for s in sims:
                state = [s.liquidity, s.slippage, s.volatility]
...
class TradeDataModule(pl.LightningDataModule):
    ...
    def __init__(self, db_url: str, batch_size: int = 64, sims_per_token: int = 10) -> None:
        super().__init__()
        self.db_url = db_url
    ...
    def setup(self, stage: str | None = None) -> None:  # pragma: no cover - simple
        ...
        self.dataset = _TradeDataset(trades, tokens, self.sims_per_token)
...
class RLTraining:
    ...
    def __init__(
        ...
        *,
        ...
        self.data = TradeDataModule(db_url, batch_size=batch_size, sims_per_token=sims_per_token)
        if algo == "dqn":
