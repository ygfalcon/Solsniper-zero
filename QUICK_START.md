...
- Run `./start.py --auto` for a fully automated launch. `run.sh` and `start.command` exec `scripts/launcher.py` on Unix and macOS respectively. It ensures the `solhunter-wallet` CLI is present, auto-selects the sole keypair and active config, verifies RPC endpoints, and warns if the wallet balance is below `min_portfolio_value`.
