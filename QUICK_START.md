...
- Run `python start.py --auto` for a fully automated launch. On macOS, double-click `start.command --auto` (which delegates to `scripts/wrap.sh`) for the same effect. It ensures the `solhunter-wallet` CLI is present, auto-selects the sole keypair and active config, verifies RPC endpoints, and warns if the wallet balance is below `min_portfolio_value`.
...
For an instant teaser covering all strategies, double-click `demo.command` (via `scripts/wrap.sh`) or
