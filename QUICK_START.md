# Quick Start
 ...
 - Run `python scripts/launcher.py --auto` for a fully automated launch. On macOS, double-click `start.command` or use `./run.sh --auto`; both forward to this Python launcher. It ensures the `solhunter-wallet` CLI is present, auto-selects the sole keypair and active config, verifies RPC endpoints, and warns if the wallet balance is below `min_portfolio_value`.
