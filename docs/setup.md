...
On macOS, double-click `start.command` (a stub invoking `scripts/wrap.sh start.py`) or run `python start.py` from Terminal for a one-click launch that executes the same Python entry point.
