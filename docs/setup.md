...
2. **Install dependencies**
  ...
  install automatically with `pip install .[uvloop]`. Running
   ...
   `python -m solhunter_zero.launcher --auto` offers identical behaviour.
...
On macOS, double-click `start.command` (or run `python -m solhunter_zero.launcher` from Terminal) for a one-click launch that executes the same Python entry point.
...
`scripts/startup.py` and the `solhunter_zero.launcher` entry point.
