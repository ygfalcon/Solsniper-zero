...
2. Run `python scripts/launcher.py --auto` (or `./start.py --auto` on Unix-like systems) or toggle **Full Auto Mode** in the GUI for a fully automated launch.
   On macOS, double-click `start.py` to invoke the Python launcher from the Finder.
   ...
   `start.py` and `make start` both forward to this launcher for consistent behaviour across platforms.
...
2. **Install dependencies**
  ...
  `python -m solhunter_zero.main --auto` (or `./start.py` on Unix) performs
...
`python -m solhunter_zero.main --auto` (or `./start.py --auto` on Unix).
...
`python -m solhunter_zero.main --auto` (or `./start.py --auto` on Unix), which performs a fully automated launch using the bundled defaults.
...
- **Missing keypair** — ensure a valid Solana keypair file is available.
  ...
  solhunter_zero.main --auto` or `./start.py` on Unix) or the Web UI select the
  sole available key automatically.
- **Service not running** — `depth_service` starts automatically with `make start` or `python -m solhunter_zero.main --auto` (or `./start.py --auto` on Unix). If it isn't responding, check logs and ensure `USE_SERVICE_EXEC`, `USE_RUST_EXEC` and `USE_DEPTH_STREAM` are all set to `True`.
