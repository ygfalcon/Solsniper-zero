...
def check_disk_space(min_bytes: int) -> None:
    ...
    try:
        ...
        log_error(f"Unable to determine free disk space: {exc}")
        raise SystemExit(1)
...
def ensure_rpc(*, warn_only: bool = False) -> None:
    ...
    if not os.environ.get("SOLANA_RPC_URL"):
        log_info(f"Using default RPC URL {rpc_url}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        log_error(f"Invalid configuration: {exc}")
        try:
            resp = input(
    ...
    if args.repair and platform.system() == "Darwin":
        ...
        for step, info in report.get("steps", {}).items():
            ...
            if msg:
                log_info(f"{step}: {info['status']} - {msg}")
            else:
                log_info(f"{step}: {info['status']}")
            if info.get("status") == "error":
                fix = mac_setup.MANUAL_FIXES.get(step)
                if fix:
                    log_info(f"Manual fix for {step}: {fix}")
    ...
    if platform.system() == "Darwin" and platform.machine() == "x86_64":
        log_warning("Warning: running under Rosetta; Metal acceleration unavailable.")
        if not args.allow_rosetta:
            log_warning("Use '--allow-rosetta' to continue anyway.")
            return 1
    ...
    log_info(f"  Config file: {config_path or 'none'}")
    log_info(f"  Active keypair: {active_keypair or 'none'}")
    log_info(f"  GPU device: {gpu_device}")
    log_info(f"  RPC endpoint: {rpc_url} ({rpc_status})")
    log_info(f"  HTTP endpoints: {endpoint_status}")
    ...
    if not args.no_diagnostics:
        ...
        log_info(f"Diagnostics summary: {summary}")
        ...
        try:
            ...
            log_info(f"Full diagnostics written to {out_path}")
